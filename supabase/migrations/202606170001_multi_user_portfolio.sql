-- Historical multi-user migration.
-- Safe with either the old `decision_journal` name or the current `investment_journal` name.
...
begin
  if (select count(*) from auth.users) = 1 then
    ...
    if to_regclass('public.decision_journal') is not null then
      update decision_journal set user_id = first_user_id where user_id is null;
    elsif to_regclass('public.investment_journal') is not null then
      update investment_journal set user_id = first_user_id where user_id is null;
    end if;
